...
class CustomDocumentationGenerator(DocumentationGenerator):
    ...
    def process_docstring(self, docstring):
        if self._obj is not None and 'core' not in inspect.getmodule(self._obj).__name__:
    ...
    for module_file in path.glob('*.py'):
        ...
        for name, member in members:
            ...
            if inspect.isclass(member):
                for cls_method_name, cls_method in inspect.getmembers(member, inspect.isroutine):
                    if cls_method_name != '__call__' and cls_method_name.startswith('_'):
                        continue
